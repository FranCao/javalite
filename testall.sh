...
Usage() {
    echo "Usage: testall.sh [options] [.jl files]"
...
Check() {
    ...
    Run "$CC" "-o" "${basename}.exe" "${basename}.s" "printbig.o" "stringrev.o" &&
...
CheckFail() {
    ...
    RunFail "$JAVALITE" "<" $1 "2>" "${basename}.err" ">>" $globallog &&
...
if [ ! -f stringrev.o ]
then
    echo "Could not find stringrev.o"
    echo "Try \"make stringrev.o\""
    exit 1
fi
...
then
    ...
    files="tests/test-*.jl tests/fail-*.jl"
fi
